...
```groovy
dependencies {
  implementation 'com.braintreepayments:popup-bridge:1.1.0'
}
```
...
```groovy
dependencies {
  implementation 'com.braintreepayments:popup-bridge:1.2.0-SNAPSHOT'
}
```
   ...
   if (window.popupBridge) {
     ...
     popupBridge.onComplete = function (err, payload) {
       if (err) {
         ...
         console.log(payload.fragment) // defaults to ""
       }
     };
   } else {
...
- The popup web page uses a deep link URL to return back to the app
  ...
  - One way to avoid hard-coding the deep link is by adding it as a query parameter to the popup URL:
    ...
    - Optionally, you can add path components, query parameters, and fragment identifiers to the deep link URL to return data to the parent page, which are provided in the payload of `popupBridge.onComplete`
